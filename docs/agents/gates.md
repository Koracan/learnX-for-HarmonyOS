...
主树（`62105e8`）算出 `13524BE3…`，`wt/t11` 那棵（编译输入与主树一致；两棵树只差几个 `.scratch/**` 的 md，都不参与编译）算出 `966051B0…`；
逐字节比较 **350 字节不同**（偏移 8–11 落在 abc 头部，其余成对出现在约 167k–169k 一段）。

- **同一棵树内是确定的**：主树连编三次都得到同一个哈希；`wt/t11` 那棵连编两次也都得到它自己的哈希。
- **与调用形式无关**：`assembleHap --no-incremental` 与带 `--mode module -p module=entry@default -p product=default` 的写法结果相同。
- ⇒ **别用跨树的哈希不等去推断"代码变了"**，也**别用"哈希相同"当跨树一致性证明**。
- 「设备上装的是哪一笔」的判据要落在**同一棵树内**的比较（例如同树 `git stash` 前后各编一次），
  或改用**内容级检索**（在 abc 里搜本次新增/删除的标识符，并配一条负对照）。这一条与上面第 3 条配套用。
- 已用过的那些**同树**对比（改前/改后同一棵树、开关翻回后重编同一棵树）**不受影响**，仍然成立。

- **但 hap 文件本身的 SHA256 不具可复现性，不能当「产物指纹」用**（2026-09-13 终验实测）：同一棵树、同一份代码连编三次
  （15:22:02 / 15:22:32 / 15:23:17），`ets/modules.abc` 三次都是 `13524BE3…`，而 **hap 文件得到三个不同的 SHA256**，
  大小也彼此不同（另一次 15:33:24 的重编是第四个哈希、大小 +5 B）。逐字节定位差异只落在这两处，
  而 **21 个 entry 的 payload SHA256 与 CRC 全部逐一相同**：
  1. 每个 entry 的 local header **偏移 10–11 的 DOS 修改时间字段**（记录构建时刻，实测 15:23:16 vs 15:33:24，逐 entry 命中）；
  2. 最后一个 entry 数据与中央目录之间那段 **`<hap sign block>`**（含 `MII…` DER；实测 35729 → 35734 B，中央目录随之整体位移）。
  ⇒ 断言「设备上装的是哪一笔」只能用 **payload 级**指纹（`ets/modules.abc` 的 SHA256；更严可逐 entry 比 payload 哈希），
  **不要用 hap 文件哈希** —— 否则会得到「每次构建都是新产物」的假象，也会把「同代码不同哈希」误读成「代码变了」。
- **构建台账**：`.hvigor/report/report-<yyyyMMddHHmmss…>.json` 每次构建一份（文件名即开始时刻，mtime 即结束时刻，
  含 `assembleHap` / `CompileArkTS` / `SignHap` 的任务记录）。要回答「某时刻的产物是谁、用哪条命令构建的」，先查这里。
