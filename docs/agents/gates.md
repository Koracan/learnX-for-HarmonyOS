...
**怎么跑**：构建耗时以分钟计（实测 45s–1m30）——不要用会阻塞的短超时前台调用，把输出重定向到 `.dsh/logs/` 或作为**后台作业**运行。
**不要用管道（`| Select-Object`）转发 `devecocli` 的输出**：会丢掉 hvigor 的失败信息，并让命令迟迟不返回。
...
- **但 hap 文件本身的 SHA256 不具可复现性，不能当「产物指纹」用**（2026-09-13 终验实测）：同一棵树、同一份代码连编三次，
  `ets/modules.abc` 三次都是同一个哈希，而 **hap 文件得到三个不同的 SHA256**、大小也彼此不同。逐字节定位差异只落在这两处：
  ① 每个 entry 的 local header 里**记录构建时刻的 DOS 时间字段**；② 最后一个 entry 数据与中央目录之间那段 **`<hap sign block>`**。
