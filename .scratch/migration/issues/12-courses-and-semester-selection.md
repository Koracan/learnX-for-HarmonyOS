...
### 边界说明（2026-09-12，由 ticket 11.5 带入）—— 课程页头改版；`取证覆盖生效` 徽标移除

- **变了什么**：
  1. 课程页头 = 「**课程 + 学期 + 相对更新时间**」（学期与更新时间在标题右侧上下两行），
     原来的 `当前学期` 前缀与"课程条数"那个裸数字**从页头移除**（第 24 条）；
     **学期文本仍然可点**，仍进学期切换页（ticket 12 的交互一行未改）；
  2. `ui_courses_override_badge`（`取证覆盖生效`）**从界面移除**——它是 ticket 12 为取证加的信号；
  3. 课程卡片右下三个计数由 emoji（🔔 📅 📁）改为 MaterialIcons `notifications` / `event` / `folder`
     （颜色仍取 `colors.outline`，数字与顺序不变）；
  4. 底色 `#FFFBFF` → `#FFFFFF`（第 25 条）。
- **你的证据还成立到哪一步**：
  - 替代验收第 1 条（切换后列表随之更新）**不受影响**；
  - 替代验收第 2 条（`aa start --ps lohSemester` 的**实际生效值进 hilog**）**一字未动**，
    `data.courses semester override: … effective="2025-2026-2" source=runtime-want-param` 与
    `data.courses snapshot semester=… source=override …` 两行**仍然在**（更强的那条证据）；
  - 替代验收第 3 条的前半"界面显示的 semester 就是实际生效值"**仍然成立**（课程页头的学期文本就是它）；
    **但"取证覆盖生效"徽标不再存在** ⇒ 覆盖生效的**界面**信号只剩这句学期文本，
    其余只能靠上面那两行 hilog。旧截图 `A3-…` / 含徽标的那几张里，徽标部分**不再可复现**。
