...
### 后置变更对本文证据的影响（统筹，2026-09-12）

ticket 04 在公告 tab 上引入了 `NavPathStack`（详情页导航），**改动了本 ticket 交付的导航外壳**。因此：

- 本文验收第 1 条（切换 tab 保留浏览位置）的证据取自该改造**之前**，严格说已被后置变更覆盖。
- 这不推翻本 ticket 当时判定的 verified（当时确实达成、证据有效），但**必须补一次确认**：切走切回 + **从详情返回列表**两个方向。
- **在补确认之前，任何人不得把"ticket 03 的滚动保持"当作当前代码已证的结论引用。**
- 统筹会在下一次设备窗口（ticket 06 的派单窗口）一并复确认并把结果补在这里。
