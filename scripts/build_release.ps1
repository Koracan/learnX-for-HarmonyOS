node -e "
  ...
  const p='harmony/AppScope/app.json5';
  const app=JSON.parse(fs.readFileSync(p));
  app.app.versionName=pkg.version;app.app.versionCode=(app.app.versionCode||0)+1;
  fs.writeFileSync(p, JSON.stringify(app,null,2)+'\n');
  "
npm run codegen
npm run patch:ohos-deps
